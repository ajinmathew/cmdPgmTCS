...
def repeat():
  print('''
  Python Program to Encrypt and Decrypt Data (DES)
  ...
  if inp=="1":
    ...
    if inp==des.keypass:
      ...
      inp=input("Do you want to reattempt (y/N) : ").upper()
      if inp=="Y":
        repeat()
  elif inp=="3": 
    print("Decrypt message without Key")
    username="admin"
    password="admin"
    ...
    if uname==username and upass==password:
      print("Login success")
      plain = des.decrypt(des.keypass,des.cipher,padding=True)
      print("Decrypted Data : {}".format(plain))
    else:
      print("Invalid Admin Credintaials")
      repeat()
